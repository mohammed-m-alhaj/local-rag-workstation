...
async def ensure_collection() -> None:
    ...
    if collection_name in names:
        try:
            coll_info = await client.get_collection(collection_name)
            vector_size = getattr(coll_info.config.params.vectors, "size", None)
            if vector_size and vector_size != settings.EMBEDDING_DIMENSION:
                logger.warning(
                    "⚠️ Qdrant collection '%s' dimension (%d) does NOT match current EMBEDDING_DIMENSION (%d). "
                    "If you changed embedding models (e.g. Gemini to nomic-embed-text), set a new QDRANT_COLLECTION in .env (e.g. documents_local) or recreate the collection.",
                    collection_name,
                    vector_size,
                    settings.EMBEDDING_DIMENSION,
                )
            else:
                logger.debug("Qdrant collection '%s' already exists (dim=%s)", collection_name, vector_size)
        except Exception as e:
            logger.debug("Qdrant collection '%s' exists (failed to inspect params: %s)", collection_name, e)
        return
